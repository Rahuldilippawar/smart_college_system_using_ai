...
if (typeof Fuse === 'undefined') {
    console.error('Fuse.js is not loaded. Ensure it is included in your project.');
}

// Responses mapped for each language
const responses = {
    'en-US': {
    ...
    'yoga session': 'Yes, yoga sessions are held daily at 6 AM in the yoga center. You’re welcome to join!',
     ...
     // New visitor queries
     'where is the main office': 'The main office is located near the entrance of the administrative building.',
     'how to get an admission form': 'You can collect an admission form from the admission office or download it from our official website.',
     'where is the canteen': 'The canteen is located near the student activity center and serves meals and snacks throughout the day.',
     'what are the canteen timings': 'The canteen is open from 8 AM to 8 PM.',
     'where is the auditorium': 'The auditorium is in the main building on the ground floor, next to the seminar hall.',
     'what are the hostel facilities': 'The hostel provides fully furnished rooms, WiFi, a study hall, and 24/7 security.',
     'how to apply for hostel accommodation': 'You can apply for hostel accommodation at the hostel office or fill out an online application on our website.',
     'where is the admission office': 'The admission office is in the main administrative building on the first floor.',
     'where is the sports complex': 'The sports complex is located on the east side of the campus and has facilities for various indoor and outdoor sports.',
     'how can I meet the principal': 'You can book an appointment with the principal through the main office reception.',
     'where can I find the timetable': 'The timetable is available on the college notice board and website.',
     'is there a medical facility on campus': 'Yes, we have a medical center located near the hostel, available 24/7 for students.',
     'where can I find the transportation schedule': 'The transport schedule is available at the main office and on the college website.',
     'where is the seminar hall': 'The seminar hall is located on the second floor of the academic block.',
     'what are the library hours': 'The library is open from 8 AM to 10 PM on weekdays and from 9 AM to 5 PM on weekends.',
    ...
    'who is the principal': 'The principal is Dr. D.D.shinde. He’s a tough cookie, but don’t worry, he has a soft spot for good grades! ',
    'who is the vice principal': 'The principal is Dr. Govind Chavhan . He’s a tough cookie, but don’t worry, he has a soft spot for good grades! ',
    ...
    'college emergency number': 'In case of emergency, call [Emergency Number]. Stay safe!',
         ...
         // HOD Queries
         'who is the HOD of computer engineering': 'The HOD of the Computer Engineering Department is Dr. Naresh Shende .',
         'who is the HOD of IT department': 'The HOD of the Information Technology Department is Dr. [Name].',
         'who is the HOD of mechanical engineering': 'The HOD of the Mechanical Engineering Department is Dr. Rupali mam.',
         'who is the HOD of civil engineering': 'The HOD of the Civil Engineering Department is Dr. Shailesh Pagar.',
         'who is the HOD of electronics engineering': 'The HOD of the Electronics Engineering Department is .',
         'who is the HOD of electrical engineering': 'The HOD of the Electrical Engineering Department is Dr. [Name].',
         'who is the HOD of artificial intelligence and data science': 'The HOD of the Artificial Intelligence & Data Science Department is Dr. [Name].',
         'who is the HOD of E&TC': 'The HOD of the Electronics and Telecommunication Engineering Department is Dr. Dr. Amit Nimbekar.',
         'who is the HOD of MBA department': 'The HOD of the MBA Department is Dr. [Name].',
         'who is the HOD of first-year engineering': 'The HOD of the First-Year Engineering Department is Dr. Omkar Sambare.',
 
    // General College Info
    'college name': 'Our college is officially called ATMA MALIK INSTITUTE OF TECHNOLOGY AND RESEARCH . ',
...
// Function to fetch the closest matching query
function getSimilarQuery(transcript, language) {
    if (!responses[language]) return 'Sorry, I did not understand that.';
    const queries = Object.keys(responses[language]);
    const fuse = new Fuse(queries, { includeScore: true, threshold: 0.5 });
    const result = fuse.search(transcript);
    return result.length ? responses[language][result[0].item] : 'Sorry, I did not understand that.';
}

// Function to suggest queries similar to the user's speech
function suggestQueries(transcript, language) {
    if (!responses[language]) return [];
    const queries = Object.keys(responses[language]);
    const fuse = new Fuse(queries, { includeScore: true, threshold: 0.6 });
    const results = fuse.search(transcript);

    const suggestions = results.slice(0, 3).map(r => r.item);
    console.log("Generated Suggestions:", suggestions); // Debugging
    return suggestions;
}

// Show related query suggestions
function showSuggestions(suggestions) {
    ...
    // Position the suggestion box below the response area
    suggestionBox.style.display = 'block';

    suggestions.forEach(query => {
        const suggestionItem = document.createElement('div');
        suggestionItem.classList.add('suggestion-item');
        suggestionItem.innerHTML = `<i class="fas fa-comment-alt"></i> ${query}`;

        // On click, set response, display, and speak it
        suggestionItem.onclick = () => {
            const response = responses[currentLanguage][query] || "Sorry, I didn't get that.";
            responseText.innerHTML = `You selected: <strong>${query}</strong><br>Response: <strong>${response}</strong>`;
...
document.addEventListener("click", (event) => {
    if (!suggestionBox.contains(event.target) && event.target !== microphoneBtn) {
...
recognition.onresult = (event) => {
    ...
    const response = getSimilarQuery(transcript, currentLanguage);
    const suggestions = suggestQueries(transcript, currentLanguage);

    console.log("Related Queries:", suggestions); // Debugging log

    responseText.innerHTML = `You said: <strong>${transcript}</strong><br>Response: <strong>${response}</strong>`;

    // Show ALL related queries below the recognized text
...
function speakResponse(response, language) {
    ...
    if (voices.length === 0) {
        setTimeout(() => {
            ...
            playVoice(response, language, voices);
        }, 500);
    } else {
        playVoice(response, language, voices);
    }
...
// Select and play the best matching voice
function playVoice(response, language, voices) {
    const preferredVoices = voices.filter(voice => voice.lang === language);
    const selectedVoice = preferredVoices.length ? preferredVoices[0] : voices[0];
...
function updateGreeting() {
    ...
    const greetings = {
        'en-US': ["Good Morning, Atma Malik!", "Good Afternoon, Atma Malik!", "Good Evening, Atma Malik!"],
        'hi-IN': ["सुप्रभात, आत्मा मालीक!", "शुभ दुपार, आत्मा मालीक!", "शुभ संध्या, आत्मा मालीक!"],
        'mr-IN': ["सुप्रभात, आत्मा मालीक!", "शुभ दुपार, आत्मा मालीक!", "शुभ संध्या, आत्मा मालीक!"]
    };
    greeting.textContent = greetings[currentLanguage][hour < 12 ? 0 : hour < 17 ? 1 : 2];
}
